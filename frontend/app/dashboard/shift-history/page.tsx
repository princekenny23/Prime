...
export default function ShiftHistoryPage() {
  ...
  useEffect(() => {
    const loadShifts = async () => {
      ...
      try {
        if (useReal && currentBusiness) {
          ...
          // Use shift history from context (simulation mode)
    ...
    loadShifts()
  }, [selectedOutlet, selectedStatus, selectedDate, shiftHistory, useReal, currentBusiness])
  ...
  const getTillName = async (tillId: string): Promise<string> => {
    try {
      if (useReal) {
        // Find outlet for this till
        const outlet = outlets.find(o => {
          // In real API, we'd need to fetch tills for each outlet
