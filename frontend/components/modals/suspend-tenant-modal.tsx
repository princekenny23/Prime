...
interface SuspendTenantModalProps {
  ...
  tenant?: any
  onSuspend?: (tenantId: string, reason: string) => Promise<void>
}

export function SuspendTenantModal({ open, onOpenChange, tenant, onSuspend }: SuspendTenantModalProps) {
  ...
  const handleSuspend = async () => {
    ...
    try {
      if (onSuspend) {
        await onSuspend(tenant.id, reason)
      }
      toast({
