...
export function PaymentModal({ 
  ...
  const handlePayment = async () => {
    if (!currentBusiness || !currentOutlet) {
      toast({
        title: "Error",
        description: "Business or outlet not selected",
        variant: "destructive",
      })
      return
    }
    ...
    if (!activeShift) {
      toast({
        title: "Error",
        description: "No active shift. Please start a shift first.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      const saleData = {
        ...
        tax: tax,
        discount: discount,
        payment_method: paymentMethod === "split" ? "cash" : paymentMethod as "cash" | "card" | "mobile" | "tab",
        notes: "",
      }
      ...
      toast({
        ...
        description: `Sale #${sale.id} completed. Total: MWK ${total.toFixed(2)}`,
      })
